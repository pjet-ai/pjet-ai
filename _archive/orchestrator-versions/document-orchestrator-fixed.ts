  ...
  try {
    ...
    if (uploadSource === 'maintenance') {
      ...
      processingResult = await processMaintenanceDocument({
        file: uploadedFile,  // ✅ Variable explícitamente definida
        uploadSource,
    ...
    // ✅ CORRECCIÓN CRÍTICA: Verificar si el procesamiento falló
...
// ✅ CORRECCIÓN: Usar GPT-4 Turbo para mejor procesamiento de texto
async function extractMaintenanceDataFromPDF(request: ProcessingRequest): Promise<ExtractedMaintenanceData> {
  console.log('🔍 OCR: Starting REAL PDF processing with OpenAI API');
  ...
  try {
    ...
    // ✅ CORRECCIÓN: Usar modelo más apropiado
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model: 'gpt-4o',  // ✅ Modelo más nuevo y capaz
        messages: [
          {
            role: 'user',
            content: [
              {
                type: 'text',
                text: 'Extract maintenance invoice data from this PDF document. Return ONLY a JSON object with these exact fields: {"date": "YYYY-MM-DD", "vendor": "string", "total": number, "currency": "string", "invoice_number": "string or null", "work_description": "string", "aircraft_registration": "string or null", "work_order_number": "string or null", "technician_name": "string or null", "location": "string or null", "labor_hours": number or null, "labor_total": number or null, "parts_total": number or null, "subtotal": number, "tax_total": number, "compliance_reference": "string or null"}. Use null for missing fields. Extract ACTUAL data from the document, do not make up values.'
              },
              {
                type: 'image_url',
                image_url: {
...
async function extractExpenseDataFromPDF(request: ProcessingRequest): Promise<ExtractedExpenseData> {
  console.log('🔍 OCR: Starting REAL expense PDF processing with OpenAI API');
        ...
        messages: [
          {
            ...
            content: [
              {
                type: 'text',
                text: 'Extract expense/receipt data from this PDF. Return ONLY a JSON object: {"date": "YYYY-MM-DD", "vendor": "string", "total": number, "currency": "string", "description": "string", "invoice_number": "string or null", "tax_amount": number, "subtotal": number, "location": "string or null", "city": "string or null", "state": "string or null"}. Use null for missing fields. Extract ACTUAL data from the document.'
              },
              {
                type: 'image_url',
                image_url: {
